...
class TestSafeConfluenceApi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher_config_url = patch('src.api.safe_confluence_api.config.CONFLUENCE_URL', 'https://mock.confluence.com')
        patcher_config_token = patch('src.api.safe_confluence_api.config.CONFLUENCE_API_TOKEN', 'mock_token')
        patcher_config_macro_name = patch('src.api.safe_confluence_api.config.JIRA_MACRO_SERVER_NAME', 'Mock Jira')
        patcher_config_macro_id = patch('src.api.safe_confluence_api.config.JIRA_MACRO_SERVER_ID', 'mock_server_id')
    ...
    def test_get_page_by_id_fallback_success(self, mock_make_request):
        ...
        mock_response.json.return_value = {"id": "FALLBACK-123", "title": "Fallback Page"}
        mock_make_request.return_value = mock_response

        result = self.safe_confluence_api.get_page_by_id("FALLBACK-123", expand="body.storage")

        self.assertEqual(result["id"], "FALLBACK-123")
        self.mock_confluence_client.get_page_by_id.assert_called_once_with("FALLBACK-123", expand="body.storage")
    ...
    def test_get_page_child_by_type_fallback_success(self, mock_make_request):
        ...
        self.mock_confluence_client.get_page_child_by_type.side_effect = Exception("Library Error")
        
        mock_response = MagicMock()
        mock_response.json.return_value = {"results": [{"id": "child1"}, {"id": "child2"}]}
        mock_make_request.return_value = mock_response
    ...
    def test_update_page_fallback_success(self, mock_make_request):
        ...
        self.mock_confluence_client.get_page_by_id.side_effect = Exception("Mock Get Page Error to force fallback")
        
        # Mock get_page_by_id used by the fallback to get current version
        mock_get_page_response = MagicMock()
        mock_get_page_response.json.return_value = {"id": "page-to-update", "title": "Old Title", "version": {"number": 5}}
    ...
    def test_create_page_fallback_success(self, mock_make_request):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {"id": "new-page-id", "title": "New Fallback Page"}
        mock_make_request.return_value = mock_response

        page_args = {"space": "TEST", "title": "New Page", "body": "<p>Content</p>", "parent_id": "parent-1"}
        ...
        mock_make_request.assert_called_once_with(
            ...
            json_data={
                ...
                "body": {"storage": {"value": page_args["body"], "representation": "storage"}},
    ...
    def test_get_user_details_by_username_fallback_success(self, mock_make_request):
        ...
        self.mock_confluence_client.get_user_details_by_username.side_effect = Exception("Library Error")
        
        mock_response = MagicMock()
        mock_response.json.return_value = {"username": "fallback_user", "displayName": "Fallback User"}
        mock_make_request.return_value = mock_response
