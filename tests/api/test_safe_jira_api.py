...
import requests # Keep for requests.exceptions for side_effect type
...
class TestSafeJiraApi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher_config_url = patch('src.api.safe_jira_api.config.JIRA_URL', 'https://mock.jira.com')
        patcher_config_token = patch('src.api.safe_jira_api.config.JIRA_API_TOKEN', 'mock_token')
    ...
    def test_get_issue_fallback_success(self, mock_make_request):
        ...
        mock_response.json.return_value = {"key": "FALLBACK-1", "fields": {"summary": "Fallback Issue"}}
    ...
    def test_create_issue_primary_success(self, mock_make_request):
        ...
        self.mock_jira_client.issue_create.assert_called_once_with(fields=issue_fields_payload["fields"])
    ...
    def test_get_available_transitions_success(self, mock_make_request):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {"transitions": [{"id": "1", "name": "To Do"}]}
        mock_make_request.return_value = mock_response
