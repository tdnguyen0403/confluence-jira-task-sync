...
class TestConfluenceTreeGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_uuid_obj_2.hex = 'task_id_uuid_xyz' # This will be sliced to [:4] in app code

        self.mock_uuid4_patcher = patch(
            'src.generate_confluence_tree.uuid.uuid4',
            side_effect=[self.mock_uuid_obj_1, self.mock_uuid_obj_2] # Provide only as many as needed by test
        )
        self.mock_uuid4 = self.mock_uuid4_patcher.start()


        # Configure mock service return values
        self.mock_confluence_service.get_user_details_by_username.return_value = {"accountId": "test_account_id"}
    ...
    def test_init(self):
        ...
        self.mock_confluence_service.get_user_details_by_username.assert_called_once_with("testuser")
    ...
    def test_generate_page_hierarchy_single_page(self, mock_datetime):
        ...
        expected_task_summary_part = f"Generated Task 0 for {expected_wp_key} ({mock_time_str})"
        ...
        self.assertEqual(jira_macro.find('ac:parameter', {'ac:name': 'key'}).get_text(strip=True), expected_wp_key,
        ...
        self.assertEqual(len(task_elements), self.mock_config.DEFAULT_TASKS_PER_PAGE, "Incorrect number of tasks generated.")
        ...
        for task_elem in task_elements:
            ...
            self.assertIsNotNone(task_id_tag.get_text(strip=True), "Task ID is empty.") # Just check presence, not exact ID
            self.assertTrue(task_summary_span.get_text(strip=True).startswith(expected_task_summary_part),
                            "Task summary does not start with expected text.")
            self.assertIsNotNone(task_assignee, "Task assignee tag missing.")
            self.assertEqual(task_assignee.get('ac:account-id'), "test_account_id", "Task assignee account ID mismatch.")
            self.assertIsNotNone(task_due_date, "Task due date tag missing.")
            self.assertEqual(task_due_date.get('datetime'), "2025-01-01", "Task due date mismatch.")
