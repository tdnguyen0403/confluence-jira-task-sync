...
class TestFastAPIDecoupledEndpoints:
    ...
    def test_sync_confluence_tasks_success(
        ...
        mock_json_dump,
        mock_open,
        mock_generate_timestamped_filename,
        mock_get_input_path,
    ...
    def test_undo_sync_run_success(
        ...
        response = client.post(
            "/undo_sync_task", json=request_payload, headers={"X-API-Key": TEST_API_KEY}
        )
    ...
    def test_update_confluence_project_success(
        ...
        mock_setup_logging,
        client,
        mock_confluence_updater_service,
    ):
        app.dependency_overrides[container.confluence_issue_updater_service] = (
            lambda: mock_confluence_updater_service
        )
        ...
        response = client.post(
            "/sync_project", json=request_payload, headers={"X-API-Key": TEST_API_KEY}
        )
    ...
    def test_api_key_unauthorized(self, client):
        ...
        valid_payload = SyncRequest(
            confluence_page_urls=["http://test.confluence.com"], request_user="test"
        ).model_dump()

        with patch.object(config, "API_SECRET_KEY", "the_real_key"):
            response = client.post(
                "/sync_task", json=valid_payload, headers={"X-API-Key": "wrong_key"}
            )
