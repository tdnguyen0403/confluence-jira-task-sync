...
# --- Tests (All 12 original tests preserved) ---
...
def test_get_relevant_jira_issues_no_type_names(updater_service, jira_stub):
    """Tests handling of cases where issue type names can't be found."""
    jira_stub.add_issue_type_name("10100", None)
    issues = updater_service._get_relevant_jira_issues_under_root(
        "PROJ-ROOT", {"10100"}
    )
    assert issues == []
    jira_stub.mock.search_issues_by_jql.assert_not_called()
...
def test_find_best_match_fuzzy_above_threshold(updater_service, mocker):
    """Tests finding a replacement with a high fuzzy summary match score."""
    mocker.patch("difflib.SequenceMatcher.ratio", return_value=0.9)
...
def test_find_best_match_fuzzy_below_threshold(updater_service, mocker):
    """Tests that no match is found if similarity is below the threshold."""
    mocker.patch("difflib.SequenceMatcher.ratio", return_value=0.5)
    old = {"fields": {"issuetype": {"id": "10100"}, "summary": "Original"}}
    candidates = [
        {
            "key": "NEW-1",
            "fields": {"issuetype": {"id": "10100"}, "summary": "Different"},
        }
    ]
    match = updater_service._find_best_new_issue_match(old, candidates, 0.7)
...
def test_find_best_match_empty_summaries(updater_service):
    """Tests that matching issue types with empty summaries are considered a match."""
    old = {"fields": {"issuetype": {"id": "10100"}, "summary": ""}}
    candidates = [
...
def test_find_and_replace_macros_success(updater_service, jira_stub):
...
def test_find_and_replace_macros_no_match(updater_service, jira_stub):
    """Tests that no change occurs if a suitable replacement isn't found."""
    html = '<p><ac:structured-macro ac:name="jira"><ac:parameter ac:name="key">OLD-1</ac:parameter></ac:structured-macro></p>'
    jira_stub.add_issue(
        "OLD-1",
        {
...
def test_update_hierarchy_no_candidates(updater_service, confluence_stub, mocker):
    ...
    assert results == []
    confluence_stub.mock.get_page_by_id.assert_not_called()
