...
class TestConfluenceIssueUpdaterService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.service = ConfluenceIssueUpdaterService(
            self.mock_confluence_service, self.mock_jira_service
        )
    ...
    def test_get_relevant_jira_issues_under_root_success(self):
        ...
        mock_search_results = [
            {
                "key": "PROJ-1",
                "fields": {
                    "issuetype": {"id": "10200", "name": "Project"},
                    "summary": "Project A",
                },
            },
            {
            ...
            },
            {
                "key": "WP-1",
                "fields": {
                    "issuetype": {"id": "10100", "name": "Work Package"},
                    "summary": "Work Package Task",
                },
            },
            {
                "key": "TASK-1",
        ...
        relevant_issues = self.service._get_relevant_jira_issues_under_root(
            root_key, target_type_ids
        )
        ...
        self.mock_jira_service.search_issues_by_jql.assert_called_once_with(
            expected_jql, fields=expected_fields
        )
    ...
    def test_get_relevant_jira_issues_under_root_no_type_names(self):
        ...
        self.mock_jira_service.get_issue_type_name_by_id.return_value = (
            None  # No names found
        )
    ...
    def test_find_best_new_issue_match_exact(self):
        ...
        candidates = [
            ...
            {
                "key": "NEW-2",
                "fields": {"issuetype": {"id": "10100"}, "summary": "Similar Summary"},
            },
        ]
    ...
    def test_find_best_new_issue_match_fuzzy_above_threshold(self):
        ...
        old_issue = {
            ...
            "fields": {
                "issuetype": {"id": "10100"},
                "summary": "Similar Summary Text Here",
            },
        }
        candidates = [
            {
                ...
                "fields": {
                    "issuetype": {"id": "10100"},
                    "summary": "Slightly Different Summary Text",
                },
            },
            {
                "key": "NEW-2",
                "fields": {
                    "issuetype": {"id": "10100"},
                    "summary": "Very Similar Summary Text Here Now",
                },
            },
        ]
    ...
    def test_find_best_new_issue_match_fuzzy_below_threshold(self):
        ...
        old_issue = {
            "key": "OLD-1",
            "fields": {"issuetype": {"id": "10100"}, "summary": "Unique Text"},
        }
        candidates = [
            {
                "key": "NEW-1",
    ...
    def test_find_best_new_issue_match_no_type_match(self):
        ...
        old_issue = {
            "key": "OLD-1",
            "fields": {"issuetype": {"id": "10100"}, "summary": "Summary"},
        }
        candidates = [
            {
                "key": "NEW-1",
                "fields": {"issuetype": {"id": "10002"}, "summary": "Summary"},
            }
        ]
        match = self.service._find_best_new_issue_match(old_issue, candidates, 0.6)
        self.assertIsNone(match)
    ...
    def test_find_best_new_issue_match_empty_summaries(self):
        ...
        old_issue = {
            "key": "OLD-1",
            "fields": {"issuetype": {"id": "10100"}, "summary": ""},
        }
        candidates = [
            {"key": "NEW-1", "fields": {"issuetype": {"id": "10100"}, "summary": ""}},
            {
                "key": "NEW-2",
                "fields": {"issuetype": {"id": "10100"}, "summary": "Not Empty"},
            },
        ]
        match = self.service._find_best_new_issue_match(old_issue, candidates, 0.6)
    ...
    def test_find_and_replace_jira_macros_on_page_success(self):
        ...
        self.mock_soup_instance.__str__ = Mock(
            return_value="<p>Some text with a task:</p>"
        ...
        self.service._find_best_new_issue_match = Mock(
            return_value=candidate_new_issues[0]
        )
        ...
        self.mock_jira_service.get_issue.assert_called_once_with(
            "OLD-1", fields="issuetype,summary"
        )
        self.service._find_best_new_issue_match.assert_called_once()

        self.assertIn("NEW-1", modified_html)
        self.assertIn(
            '<ac:structured-macro ac:name="jira"><ac:parameter ac:name="key">NEW-1</ac:parameter></ac:structured-macro>',
            modified_html,
        )
    ...
    def test_find_and_replace_jira_macros_on_page_no_match(self):
        ...
        mock_html_content = '<p><ac:structured-macro ac:name="jira"><ac:parameter ac:name="key">OLD-1</ac:parameter></ac:structured-macro></p>'
        mock_page_details = {"id": "page1", "title": "Test Page"}
    ...
    def test_update_confluence_hierarchy_with_new_jira_project_success(self):
        ...
        self.service._get_relevant_jira_issues_under_root = Mock(
            return_value=mock_candidate_issues
        )

        mock_page_details_root = {
            "id": "root_page_id",
            "title": "Root Page",
            "body": {"storage": {"value": "OLD-1 macro"}},
        }
        mock_page_details_child = {
            "id": "child_page_id",
            "title": "Child Page",
            "body": {"storage": {"value": "OLD-2 macro"}},
        }
        ...
        self.service._find_and_replace_jira_macros_on_page = Mock(
            side_effect=[("MODIFIED_HTML_ROOT", True), ("MODIFIED_HTML_CHILD", False)]
        )
        ...
        self.mock_confluence_service.get_page_id_from_url.assert_called_once_with(
            root_url
        )
        self.mock_confluence_service.get_all_descendants.assert_called_once_with(
            "root_page_id"
        )
        ...
        self.assertEqual(
            self.service._find_and_replace_jira_macros_on_page.call_count, 2
        )
        self.mock_confluence_service.update_page_content.assert_called_once_with(
            "root_page_id", "Root Page", "MODIFIED_HTML_ROOT"
        )
