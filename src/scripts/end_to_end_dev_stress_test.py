# To test high number of tasks and deeply nested pages
...
async def run_end_to_end_test() -> None:
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=60.0) as client:
        ...
        try:
            ...
            sync_task_responses_list = sync_task_full_response[
                "jira_task_creation_results"
            ]
            confluence_update_list = sync_task_full_response[
                "confluence_page_update_results"
            ]

            assert isinstance(
                sync_task_responses_list, list
            )  # This assertion now correctly checks the inner list
            ...
            jira_creation_results_for_undo = [
                ...
                for res in sync_task_responses_list
                if res["success"]
            ]
            ...
            for result_for_undo in jira_creation_results_for_undo:
                assert (
                    "new_jira_task_key" in result_for_undo
                    and result_for_undo["new_jira_task_key"] is not None
                )
                ...
                assert sync_task_full_response["overall_jira_task_creation_status"] in [
                    "Success",
                    "Partial Success",
                ]
